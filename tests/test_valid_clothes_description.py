...
def test_correct_clothes_description_4():
    assert validator.check_clothes_description('In excellent condition') == True
...
def test_correct_clothes_description_7():
    assert validator.check_clothes_description('Платье вечернее длинное') == True
...
def test_correct_clothes_description_11():
    assert validator.check_clothes_description('Очень красивая и стильная вещь для особых случаев') == True
